 ...
 import com.zt.service.ImportZTZItemService;
 ...
 @Service
 public class ImportZTZItemServiceImpl
   implements ImportZTZItemService
 {
   @Autowired
   ...
   public String importZTZItem(String jsonStr) {
     String result = "fail";
     ...
     try {
       ...
       if (StringUtils.isNotEmpty(param)) {
         ...
         for (int i = 0; i < orderJSONArray.size(); i++) {
           ...
           if (StringUtil.isEmpty(companycode).booleanValue()) {
             message.append("推送失败，失败原因：商家编码没有输入或不正确！");
           ...
           String specifications = orderJSONObject.getString("itemSpec");
           
           String unit = orderJSONObject.getString("unitDesc");
           
           String sku = orderJSONObject.getString("itemSKU");
           if (StringUtils.isEmpty(sku)) {
             message.append("推送失败，失败原因：料号没有输入！");
             
             break;
           } 
           String weight = orderJSONObject.getString("unitWeight");
