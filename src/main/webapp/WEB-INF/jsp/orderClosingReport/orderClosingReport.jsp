...
$(function(){
	$("#D_closingReport").datagrid({
        url        :'<%=path%>/orderClosingReport/findAll.do',
        columns    :[
            [
...
var select = document.getElementById("reportNPUT");
...
var monthChoose = document.getElementById("reportYPUT");
...
function outDetailXLSX(){
	 ...
	 if (row.length == 0) {
		...
		for (var i = 0; i < row.length; i++){
    		...
    		var companynos = companynos + row[i].companyno+"'" +","+"'";
        }
		$('#orderClosingCompanyCode').val(companynos);
...
<div>
	<form id="find_closingReport">
		...
		<input  name="tOrderClosing.type" id="type"hidden="" >
		<input  name="ids" hidden=""id="ids">
